...
>
> **Cập nhật lần cuối**: 2026-04-20 · **Phiên bản hiện tại**: `MomentumV1 v1.1`
>
...
**Phiên bản algo**: `MomentumV1 v1.1` — thêm **5 hard gates / enhancements** so với v1.0: MACD zero-line, RS vs basket, adaptive TP regime, breakeven stop, sector limit.

### 2.1 Aggregate Metrics — So sánh qua các phiên bản

| Metric | v1.0 Before | v1.0 After (Top-3) | v1.1 (Round-2) | Mục tiêu | Status |
|--------|------------:|-------------------:|---------------:|---------:|:------:|
| Total return | +3.69% | +7.05% | **−2.88%** | > 50% / 4 năm | ❌ |
| CAGR | +0.92% | +1.74% | **−0.74%** | > 10%/năm | ❌ |
| Sharpe | 0.141 | 0.261 | **−0.079** | > 1.0 | ❌ |
| Sortino | 0.127 | 0.211 | **−0.061** | > 1.5 | ❌ |
| Max drawdown | 17.21% | **8.62%** | 9.86% | < 12% | ✅ |
| Win rate | 28.63% | 36.74% | **42.73%** | > 40% | ✅ |
| Profit factor | 1.038 | 1.106 | **0.957** | > 1.5 | ❌ |
| Avg win | +7.53M | +4.58M | +3.37M | — | — |
| Avg loss | −2.91M | −2.41M | −2.63M | — | — |
| R:R thực tế | 2.59 | 1.90 | **1.28** | ≥ 2.0 | ❌ |
| Tổng trades | 227 | 215 | 227 | ≥ 150 | ✅ |
| Avg hold | 21.7 ngày | 13.7 ngày | **9.5 ngày** | 5-25 ngày | ✅ |

**Phân loại exit (v1.1)**:
| Reason | Count | % |
|--------|------:|--:|
| STOP (hit hard stop, chưa activate trail) | 108 | 47.6% |
| TRAIL (hit trailing stop sau +1R) | 44 | 19.4% |
| TP (adaptive TP SIDEWAYS/VOLATILE) | 74 | 32.6% |
| EOD (close cuối kỳ) | 1 | 0.4% |

### 2.2 Phân rã theo năm

| Năm | Return | Trades | Win Rate | PnL (M VND) |
|-----|-------:|-------:|---------:|------------:|
| 2022 (Q2-Q3) | +0.63% | 4 | 50.00% | +3.1 |
| 2023 | −3.89% | 50 | 38.00% | −19.6 |
| 2024 | −1.54% | 51 | 39.22% | −8.6 |
| 2025 | **+5.56%** | 92 | 50.00% | +26.5 |
| 2026 (YTD) | −3.38% | 30 | 33.33% | −16.1 |

### 2.3 Phân rã theo quý (v1.1)

| Quarter | Return | Trades | WinRate | PnL (M VND) |
|---------|-------:|-------:|--------:|------------:|
| 2022Q2 | 0.00% | 0 | — | 0.0 |
| 2022Q3 | +0.63% | 4 | 50.00% | +3.1 |
| 2022Q4 | 0.00% | 0 | — | 0.0 |
| 2023Q1 | −1.63% | 8 | 37.50% | −5.9 |
| 2023Q2 | +1.30% | 16 | 50.00% | +3.3 |
| 2023Q3 | −1.07% | 21 | 38.10% | −4.6 |
| 2023Q4 | −2.50% | 5 | 0.00% | −12.4 |
| 2024Q1 | **+3.56%** | 11 | 54.55% | +1.9 |
| 2024Q2 | −3.19% | 16 | 31.25% | −0.0 |
| 2024Q3 | +0.48% | 10 | 50.00% | +3.1 |
| 2024Q4 | −2.27% | 14 | 28.57% | −13.6 |
| 2025Q1 | −0.12% | 24 | 45.83% | +3.1 |
| 2025Q2 | −0.64% | 14 | 50.00% | −6.1 |
| 2025Q3 | **+7.25%** | 30 | 60.00% | +36.1 |
| 2025Q4 | −0.83% | 24 | 41.67% | −6.6 |
| 2026Q1 | −3.99% | 28 | 32.14% | −19.0 |
| 2026Q2 | +0.63% | 2 | 50.00% | +2.9 |

**Quý âm: 9/17** (không đổi so với v1.0 After). 2022 cải thiện (+0.63% vs −0.13%) nhờ macro filter. 2026Q1 tệ hơn (−3.99% vs −3.59%).

### 2.4 Quan sát chính (v1.1)

1. **Win rate vượt mục tiêu 40% lần đầu tiên** — 42.73% (+6pp so với v1.0). MACD gate + RS gate lọc hiệu quả false-positive signal, nhưng…
2. **Profit factor giảm xuống < 1.0** — vấn đề chính: avg_win giảm từ 4.58M → 3.37M (−26%). Nguyên nhân: adaptive TP SIDEWAYS (2×ATR) cắt sớm nhiều trade mà trước đây sẽ chạy tiếp theo trailing stop. TRAIL exits giảm từ 105 → 44 (-58%), TP exits tăng từ 0 → 74. Net effect: nhiều wins nhỏ hơn.
3. **RS gate làm tổng số trades tăng về 227 (từ 215)** — sector limit thay thế bank-dominated positions bằng non-bank symbols có quality thấp hơn. Bank sector (11 symbols/VN30) bị cap 2 slots → pass nhiều bank leaders, take thêm non-bank trades dù quality thấp.
4. **Adaptive TP regime quá tight** — SIDEWAYS: 2×ATR đã cắt nhiều winner. Cần test SIDEWAYS TP = 3×ATR hoặc loại adaptive TP cho SIDEWAYS (chỉ giữ VOLATILE = 1.5×ATR). Đây là vùng cần tune tiếp.
5. **MACD zero-line gate isolated impact nhỏ** — kết hợp với RS gate, hai gate này cùng lọc là tốt nhưng cần isolate để đo từng impact riêng.
6. **Breakeven stop tác động nhỏ nhưng đúng chiều** — avg_loss −2.41M → −2.63M (tệ hơn chút) do một số trade mà trước đây TRAIL exit tại stop < entry, giờ breakeven tại entry → loss giảm nhưng avg ảnh hưởng bởi sample thay đổi.
7. **⚠️ v1.1 tệ hơn v1.0 overall** — cần isolate từng cải tiến, tune adaptive TP params, và test không có sector limit hoặc với bank cap = 3. Backlog: parameter sensitivity sweep cho `TP_SIDEWAYS_MULT ∈ {2, 3, 4}` và `RS_GATE_ON/OFF`.
...
- [x] **Relative strength vs VN30 basket** — chỉ BUY khi return(20d) của symbol > return(20d) của basket *(commit pending — chạy backtest để đo impact)*
...
- [x] **TP thích nghi theo regime** — TRENDING: trailing stop (không đổi); SIDEWAYS: exit ATR×2; VOLATILE: exit ATR×1.5 *(commit pending)*
  - **Rationale**: Sideways cho lại profit nhanh — chốt sớm. Nhiều quý (2023Q3, 2024Q2-Q4) lỗ do thị trường sideways.
  - **Implementation**: `pending_buys` giờ mang thêm `regime`; `Position.take_profit` set tại entry (None nếu TRENDING). Section C kiểm tra `hi >= take_profit` trước trailing ratchet. Áp dụng nhất quán trong `backtest_portfolio_vn30.py` và `core/backtester.py`.
  - **Success**: WR trong SIDEWAYS ≥ 35%, PF trong SIDEWAYS > 1.0
- [x] **Breakeven stop sau +1R** — kéo stop về entry khi đạt +1R (bảo vệ chống lỗ) *(commit pending)*
...
- [x] **Giới hạn concentration theo sector** — tối đa 2 vị thế mở cùng sector *(commit pending)*
...
**Round-2 đã implement** (6 items) nhưng **net negative**: WR tăng lên 42.73% ✅ nhưng PF giảm 1.106 → 0.957 ❌, CAGR −0.74% ❌. Vấn đề chính: adaptive TP SIDEWAYS 2×ATR cắt sớm winners, sector limit thay bank leaders bằng non-bank thấp hơn.

**Vòng tiếp theo: parameter tuning và tách isolate**:

1. 🔥 **Tune adaptive TP SIDEWAYS** — test `TP_SIDEWAYS_MULT ∈ {3, 4, 4.5}` để tìm điểm cân bằng win-cut vs chất lượng win.
2. 🔥 **Isolate từng gate** — chạy backtest với chỉ MACD gate (không RS, không adaptive TP) để đo isolated impact, sau đó thêm RS gate riêng.
3. **Tune sector limit** — test `max_per_sector ∈ {2, 3}` và bank riêng vs uniform; bank cap = 3 có thể tốt hơn.
4. **Parameter sensitivity grid** (3.7) — sweep `BUY_THRESHOLD ∈ {0.45-0.65}` × `ATR_TP_MULT` × các gate ON/OFF để tìm robust plateau.
...
|------|----------|--------|---------|
| 2026-04-20 | **Re-baseline VN30 portfolio** sau Round-2 (v1.1) | `228cda5`–`2ad3aee` | WR 36.74% → 42.73% ✅, nhưng PF 1.106 → 0.957 ❌, CAGR +1.74% → −0.74% ❌. Adaptive TP quá tight + sector limit thay thế bank leaders. Cần parameter tuning. |
