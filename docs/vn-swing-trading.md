...
6. **Slot utilization 67.7% — không "trống", mà đã bottleneck 40% thời gian**. Mean 3.37/5 positions mở; 405/999 ngày (40.5%) đã maxed 5/5 slot, signal mới bị reject do ranking. **Không phải raise max_positions giải quyết được** — edge hiện tại quá thấp (PF 1.038, WR 28.63%), thêm slot = thêm losing trades, ngoài ra 500M/10 slot = 50M/slot quá nhỏ so với lot VN (1 lot ≈ 5-20M) và tương quan nội ngành cao (6-7 ngân hàng, 4-5 BĐS trong VN30) sẽ làm MDD tăng không tuyến tính. Khi nào WR ≥ 40% và PF ≥ 1.3 mới nên cân nhắc raise slot để scale edge.
...
- [x] **VN30 macro regime filter** — basket VN30 equal-weight, EMA50; chặn BUY mới khi basket ≤ basket EMA50 *(commit `8c7d3c7` — impact đo sau re-baseline)*
...
- [x] **Xác nhận volume breakout** — hard gate `vol >= 1.5 × vol_MA20` trên signal bar *(commit pending — cập nhật sau re-baseline)*
...
- [x] **Trailing stop sau +1R** — khi unrealized gain > 1R (= 1.5×ATR), trail stop tại `high - 2×ATR`; bỏ TP cố định *(commit pending — impact đo sau re-baseline)*
...
|------|----------|--------|---------|
| 2026-04-19 | Trailing stop sau +1R (roadmap 3.3) | *(pending)* | Bỏ TP cố định; `ATR_TRAIL_TRIGGER=1.5`, `ATR_TRAIL_MULT=2.0`; stop ratchet only up |
